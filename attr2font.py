...
class AttributeAwareUp(nn.Module):
    ...
    def forward(self, x, skip_input, attr_feature):
        ...
        #print(x.shape, attr_feature.shape)
...
class AttrClassifier(nn.Module):
    def __init__(self, in_channel=3, attr_channel=13):
...
class DiscriminatorWithClassifier(nn.Module):
    def __init__(self, in_channel=3, attr_channel=13, pred_attr=True):
    ...
    def forward(self, img_A, img_B=None, attr_B_intensity=None):
        if img_B != None:
            if self.pred_attr:
            ...
            if self.pred_attr:
                out = self.model_one_input(img_A)
                out_rf = self.f(out)  # real or fake for image
                out_A_attr = self.auxiliary_cls(img_A)
                return out_rf, out_A_attr