...
class AttGAN():
    def __init__(self, args, vgg_network):
        self.mode = args.mode
        self.gpu = args.gpu
        ...
        self.attrid = self.attrid.repeat(args.batch_size, 1)
        
        if self.multi_gpu:
            self.G = nn.DataParallel(self.G)
            self.D = nn.DataParallel(self.D)
            self.attribute_embed = nn.DataParallel(self.attribute_embed)
        
        self.optim_G = optim.Adam([
            {'params': self.G.parameters()},
            {'params': self.attribute_embed.parameters(), 'lr': 1e-3}],
    ...
    def trainG(self, img_a, att_a, att_a_, att_b, att_b_, img_B):
        ...
        # Construct attribute
        attr_raw_A = self.attribute_embed(self.attrid)
        attr_raw_B = self.attribute_embed(self.attrid)

        attr_A_intensity = att_a_
        attr_A_intensity_u = attr_A_intensity.unsqueeze(-1)
        attr_A = attr_A_intensity_u * attr_raw_A

        attr_B_intensity = att_b_
